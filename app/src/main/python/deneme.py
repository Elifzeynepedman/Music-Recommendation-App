...
tanıyıcı = cv2.face.LBPHFaceRecognizer_create() # yüz tanıyıcı oluşturuldu.
...
def getImagesAndLabels(path): # görüntüler ve tanımlar için yol atandı.
    ...
    for imagePaths in imagePaths: # for döngüsü oluşturuldu.
        # resimin okunacağı dosyayı açmak için PIL_img 'ten yararlanıldı.
        PIL_img = Image.open(imagePaths).convert('L')
        img_numpy = np.array(PIL_img,'uint8') # pozitif tamsayı olduğu belirtildi.
        id = int(os.path.split(imagePaths)[-1].split(".")[1]) # isimin yazılacağı yer belirlendi.
        print(id)
...
yuzler,isimler = getImagesAndLabels('veri') # resimlerin bulunduğu klasör adı girildi.
tanıyıcı.train(yuzler, np.array(isimler)) # tanıyıcı deneme oluşturuldu.
tanıyıcı.save('deneme/deneme.yml') # kaydedilcek klasör belirlendi.
